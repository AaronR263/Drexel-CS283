...
int get_student(int fd, int id, student_t *s)
{
    ...
    if (rc == 0 || fstudent.id != id) {
        return SRCH_NOT_FOUND;
    }

    memcpy(s, &fstudent, STUDENT_RECORD_SIZE);
    return NO_ERROR;
}
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    new_student.id = id;
    new_student.gpa = gpa;
    strncpy(new_student.fname, fname, sizeof(empty_student.fname));
    strncpy(new_student.lname, lname, sizeof(empty_student.lname));
...
int count_db_records(int fd)
{
    ...
    int offset = STUDENT_RECORD_SIZE;
    lseek(fd, offset, SEEK_SET);

    student_t database[MAX_STD_ID];
    
    int bytes_read;
    student_t fstudent = {0};
    while ((bytes_read = read(fd, &fstudent, STUDENT_RECORD_SIZE)) != 0) {
         
        if (memcmp(&fstudent, &empty_student, STUDENT_RECORD_SIZE) != 0)
              cntr++;
    }
